...
(async () => {
  ...
  await Promise.all(list.map(async (k) => { try {
    if (job === 'edits') { await editImage(`${H}/${k}-base.jpg`, EDITS[k], `${H}/${k}-edit${SUF}.jpg`, { model: 'fal-ai/flux-pro/kontext/max', guidance: 3 }); console.log('edit ok', k); }
      ...
      const r = await falRun('fal-ai/veo3.1/image-to-video', { prompt: MOTION + (EXTRA[k] || ''), image_url: url, aspect_ratio: '16:9', duration: '8s', resolution: '1080p', generate_audio: false }, { timeoutMs: 1500000 });
